...
import { AssetId, addressFromBech32, NetworkId } from "@blaze-cardano/core";
...
// Blockfrost game state manager for simulation
class BlockfrostGameStateManager extends GameStateManager {
    ...
    async loadRealPellets(): Promise<void> {
        console.log("🔍 Querying real pellets from Blockfrost...");
...
class BlockfrostPathfindingSimulator {
    private gameState: BlockfrostGameStateManager;
    private spawnOptimizer: SpawnOptimizer;
    private pathfinder: Pathfinder;
    private fuelManager: FuelManager;
    private provider: Blockfrost;
    ...
    async initialize(): Promise<void> {
        console.log("🔧 Initializing simulation with Blockfrost...");
        
        // Check environment variables
        const projectId = process.env.BLOCKFROST_PROJECT_ID;
        if (!projectId) {
            throw new Error("Missing required environment variable: BLOCKFROST_PROJECT_ID");
        }
        ...
        console.log(`   Pellet validator: ${pelletValidatorAddress.toBech32()}`);
        console.log(`   Fuel token: ${fuelToken}`);

        // Initialize game state manager with Blockfrost
        this.gameState = new BlockfrostGameStateManager(
            this.provider,
        ...
        console.log("✅ Simulation initialized with Blockfrost connection");
    }

    async runSimulation(): Promise<void> {
        console.log("🧪 Asteria Pathfinding Simulation (Blockfrost)");
        console.log("===============================================\n");

        // Initialize components
        await this.initialize();
        
        // Load real pellet data from Blockfrost
    ...
    private async setupTestScenario(): Promise<void> {
        console.log("🔍 Loading real game state from Blockfrost...");
        
        try {
            // Load real pellets from Blockfrost
            await this.gameState.loadRealPellets();
            
            const stats = this.gameState.getPelletStats();
            console.log(`📡 Blockfrost blockchain state:`);
            ...
            if (stats.totalPellets === 0) {
                ...
                console.log(`✅ Using real pellets from Blockfrost:`);
            ...
            console.error("❌ Failed to load real pellets from Blockfrost:", error);
            console.log("🎲 Falling back to mock pellets for testing...");
            this.addTestPellets();
        }
    }
    ...
    // Copy the rest of the simulation methods from the original simulator
    ...
    private showResults(): void {
        ...
        if (success) {
            console.log("\n🎉 Pathfinding algorithm SUCCESS with Blockfrost!");
...
// Run the simulation
async function main() {
    const simulator = new BlockfrostPathfindingSimulator();
    
    try {
        await simulator.runSimulation();
    } catch (error) {
        console.error("❌ Blockfrost simulation failed:", error);
        console.log("\n💡 Make sure to set up your environment variable:");
        console.log("   BLOCKFROST_PROJECT_ID=your_blockfrost_project_id");
        process.exit(1);
    }
}
...
export { BlockfrostPathfindingSimulator };